...
def fit_one_epoch(G_model_train, D_model_train, G_model, D_model, G_optimizer, D_optimizer, BCE_loss, 
    ...
    #----------------------------#
    if (epoch + 1) % 10==0:
        # torch.save(G_model.state_dict(), 'results/NEU_DET/scratches/pth/G_Epoch%d-GLoss%.4f-DLoss%.4f.pth'%(epoch + 1, G_total_loss / (epoch_step + 1), D_total_loss / (epoch_step + 1)))
        # torch.save(D_model.state_dict(), 'results/NEU_DET/scratches/pth/D_Epoch%d-GLoss%.4f-DLoss%.4f.pth'%(epoch + 1, G_total_loss / (epoch_step + 1), D_total_loss / (epoch_step + 1)))
        save_dir = 'results/MTD/Blowhole_temp/pth/'
        os.makedirs(save_dir, exist_ok=True)  # 自动创建目录
        # 保存生成器模型
        G_path = os.path.join(save_dir,f'G_Epoch{epoch + 1}-GLoss{G_total_loss / (epoch_step + 1):.4f}-DLoss{D_total_loss / (epoch_step + 1):.4f}.pth')
        torch.save(G_model.state_dict(), G_path)
        # 保存判别器模型
        D_path = os.path.join(save_dir,f'D_Epoch{epoch + 1}-GLoss{G_total_loss / (epoch_step + 1):.4f}-DLoss{D_total_loss / (epoch_step + 1):.4f}.pth')
